...
async function buildInstagramInbox({ limit = 50, commentsLimit = 50 } = {}) {
  ...
  const mediaLimit = Math.min(Math.max(Number(limit) || 50, 1), 100);
  const commentLimit = Math.min(Math.max(Number(commentsLimit) || 50, 1), 100);
  const mediaData = await graphFetch(config, `${config.accountId}/media`, {
